...
let imgsrc =ref<string>(staticData.verifyImg)
...
let verifyImg = ref<any>(null)
let verifyImg1 = ref<any>(null)
  ...
  <NTabs v-model:value="tabStr" justify-content="space-evenly" type="line">
    ...
    <NTabPane name="register" tab="注册">
      <NForm ref="formRef" :model="registerForm" :style="{ maxWidth: '640px' }" class="auto" style="margin:0 auto;">
        ...
        <NFormItem label="滑块验证" :rule="{
          required: true,
        }" 
        v-if="verifyFlag.showLogin"
        >
          <dragVerifyImgChip :imgsrc="imgsrc"
            ref="verifyImg"
            v-model:isPassing="verifyFlag.loginFlag" showRefresh :barWidth="40" text="请按住滑块拖动" successText="验证通过"
            handlerIcon="material-symbols:keyboard-double-arrow-right" successIcon="clarity:success-standard-solid" refreshIcon="ic:twotone-refresh"
            @refresh="refresh('loginFlag')" @passcallback="passcallback('loginFlag')">
          </dragVerifyImgChip>


        </NFormItem>
        <NFormItem label="邀请人邮箱（双方都可获得50000字符额度）" path="email" :rule="{
          required: false,
          message: '',
          trigger: ['input', 'blur'],
        }">
    ...
    <NTabPane name="forget" tab="重置密码">
      <NForm ref="formRef" :model="forgetForm" :style="{ maxWidth: '640px' }" class="auto" style="margin:0 auto;">
        ...
        <NFormItem label="邮箱验证码" path="emailCode" :rule="{
          ...
          v-if="verifyFlag.showForget"
          >
            <dragVerifyImgChip :imgsrc="imgsrc"
              ref="verifyImg1"
              v-model:isPassing="verifyFlag.forgetFlag" showRefresh :barWidth="40" text="请按住滑块拖动" successText="验证通过"
              handlerIcon="material-symbols:keyboard-double-arrow-right" successIcon="clarity:success-standard-solid" refreshIcon="ic:twotone-refresh"
