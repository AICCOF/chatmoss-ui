  ...
  <Page>
    ...
    ">
      <div style="
        ...
        text-align: center;
      ">
        <div style="color: #fff; font-size: 24px; font-weight: 600; line-height: 70px">ChatMoss 商店</div>
      </div>

      <div class="flex-1" style="overflow-y: auto">
        <div style="box-sizing: border-box; margin: 0 auto; margin-top: 30px; padding: 0 28px">
          <div class="title-h1"> 兑换码（登录后才能兑换哦~） </div>
          <div class="flex">
            <Input v-model="toMossCode" class="mr-2 input" type="text" placeholder="请输入您的兑换码" style="" />
            <div class="btn-exchange" @click="toMossEvent"> 兑换 </div>
          </div>
        </div>
        ...
        <div>
          <div class="flex-center" style="
            ...
            line-height: 54px;
          ">
            <div class="btn" :class="[activeName1 == '1' ? 'active' : '']" @click="handleTabs('1')">商城</div>
            <div class="btn" :class="[activeName1 == '2' ? 'active' : '']" @click="handleTabs('2')">购买记录</div>
          </div>
          <div>
            <div v-show="activeName1 == '1'" style="padding: 0 28px">
              <Tabs v-model="activeName" class="shop-content">
                <TabPane v-for="item of list" :key="item.index" :tab="item.grouTtitle" :name="item.index">
                  <div v-for="(row, index) in item.products" :key="index" class="card"
                    :class="[index < item.products.length - 1 ? 'border' : '']">
                    <div>
                      <div>
                        <div class="title">{{ row.title }}</div>
                        <div class="desc mt-1">{{ row.desc }}</div>
                        <div class="mt-1 flex">
                          <div v-for="(tagItem, tagIndex) in row.tags" :key="tagIndex" class="tag">
                            {{ tagItem }} &nbsp;
                          </div>
                        </div>
                        <div class="flex-center" style="margin-top: 14px"><span class="price mr-1">¥<span
                              style="margin-left: 6px; font-size: 20px; font-weight: bold">{{
                                row.price / 100
                              }}</span></span><span class="originPrice" style="text-decoration: line-through">
                            <span>¥</span>
                            <span>{{ row.originPrice / 100 }}</span></span></div>
                      </div>
                    </div>

                    <div class="text-center">
                      <div v-for="data of payTypeInfo.list" :key="data.payType" class="mt-2">
                        <div :class="data.payType === 'wx-native' ? 'btn_success' : 'btn_primary'"
                          @click="handlWechat(row, data)">{{ payTypeInfo.textMap[data.payType] }}</div>
                      </div>
                    </div>
                  </div>
                </TabPane>
              ...
              <div v-if="orderLists.length !== 0">
                ...
                <div v-for="(row, index) in orderLists" :key="index" class="order_list">
                  ...
                  <div class="flex-center" style="justify-content: space-between; margin-top: 2px">
                    <span>订单编号</span>
                    <div class="flex-center" style="color: #717172"><span>{{ row.payOrderNo }}</span>
     ...
     <Modal v-model:visible="show" class="self-model" :centered="true" :title="null" :footer="null" 
          ...
          <div style="width: 416px" class="flex items-center justify-center">
            <div>
              ...
              <div class="mt-2 text-center" style="margin-top: 24px">{{ alertInfo.title }}</div>
              <div><img class="zf-img" :src="orderInfo.payParam.url" alt=""
                  style="display: block; width: 170px; margin: 0 auto; margin-top: 0" /></div>
              <div class="text-center" style="margin-top: 30px; margin-bottom: 20px; color: red; font-size: 16px">需支付：{{
                alertInfo.price / 100 }}</div>
              <div class="tip-text-input1">虚拟商品，不支持退款，请再三考虑是否需要购买</div>
            </div>
          </div>
        </Modal>
        <Modal v-model:visible="show1" :centered="true" :title="null" :footer="null" class="self-model" 
          style="width: fit-content" @cancel="handleClose">
          <div class="flex items-center justify-center" style="width: 416px">
            ...
            ">
              ...
              <div class="mt-2 text-center" style="margin-top: 24px">{{ alertInfo.title }}</div>
              <div><img :src="alertInfo.microImg" class="mt-2 img-center" alt=""
                  style="display: block; width: 170px; margin: 0 auto; margin-top: 20px" /></div>
              <div class="text-center" style="margin-top: 30px; margin-bottom: 20px; color: red; font-size: 16px">需支付：{{
                alertInfo.price / 100 }}</div>
              <div class="tip-text-input1">虚拟商品，不支持退款，请再三考虑是否需要购买</div>
            </div>
...
import { message, Modal, Input, Select, TabPane, Tabs, Empty } from 'ant-design-vue';
...
async function exchangeMossEvent() {
  if (!exchangeMossCode.value) return;
